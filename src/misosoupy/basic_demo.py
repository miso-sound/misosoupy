...
with open(categorySpreadsheetFile,"r") as t:
    for line in t:
        ...
        if len(txtArray)==0: #if first row, save as headers
            ...
            txtArray=np.vstack([txtArray,[soundCategory,soundLabel,soundSource,soundFile]]) #line_clean)      
...
def presentInstructions(instructionText,waitTime):
    ...
    # Prep instructions 
    instructTxt1 = visual.TextStim(win, text=instructionText, pos=(0,0), color=textColor, height=0.09, wrapWidth=6) #height=0.12, alignText='Left')
    ...
    continueText = visual.TextStim(win, text='Click here to continue', pos=(0.7, -.85), color=screenColor, height=0.08) 
    exitShape = visual.ShapeStim(win, vertices=((-.5, -.3), (-.5, .3), (.5, .3), (.5, -.3)), pos=(1, 1), size=(.35, .35), opacity=100, fillColor=screenColor, lineColor=None,lineWidth=4.0,name='exitShape')
    ...
    while continueChosen == False:
        ...
        continueShape = visual.ShapeStim(win, vertices=((-.5, -.3), (-.5, .3), (.5, .3), (.5, -.3)), pos=(0.7, -.85), size=(.45, .2), opacity=100, fillColor=continueShapeColor, lineColor=shapeLineColor,lineWidth=4.0,name='continueShape')
        continueText = visual.TextStim(win, text='Click here to continue', pos=(0.7, -.85), color=textColor, height=0.08) 
...
def presentItemList(pageNum,pauseTime,instr1,instr2,instr2color,instr3):
    ...
    xPosCenter = (2/numColumnsPerPage)/2-.5*1.25 #2/ since distance of screen units (+1-->-1), /2 for middle of word, *1.5 for scale
    yPosCenter = ((numItemsPerColumn*itemHeight)/2)+itemHeight*4 #items*height gives total screen needed, /2 to split equally b/w top and bottom half of screen
  
    allWordPosValues=[]
    allSquarePosValues=[]
    for iXpos in range(numColumnsPerPage):
        currXpos=xPosCenter+.1+.6*(iXpos) #first column starts at center, next shifts right
        for iYpos in range(numItemsPerColumn):
            currYpos=yPosCenter-(squareOutlineSize/2)-squareOutlineSize*1.25*(iYpos)
               
            allWordPosValues.append((currXpos+0.35,currYpos)) #[iYpos]))
            allSquarePosValues.append((currXpos-.2,currYpos))
   
    
    currPageItems=uniqueSoundLabels[0+iPage*numItemsPerPage:numItemsPerPage+iPage*numItemsPerPage]
    allScreenWords = []
    allBoxes = []
    for iItem in range(0,len(currPageItems)): #(totalPicCount,totalPicCount+(numPicRows*numPicsPerRow)): #len(allDistTrialStim)):
        if doneWithMostTriggering:
            if currPageItems[iItem] in mostTriggeringList: #make sounds already chosen unclickable
                allScreenWords.append(visual.TextStim(win, text=currPageItems[iItem].replace('_',' '), pos=allWordPosValues[iItem], color='gray', height=itemHeight, alignText='Left'))
                allBoxes.append(visual.ShapeStim(win, vertices=((-.5, -.3), (-.5, .3), (.5, .3), (.5, -.3)), pos=allSquarePosValues[iItem], size=(0,0), opacity=100, fillColor=None,lineColor=shapeLineColor,lineWidth=3.0))
            else:
                allScreenWords.append(visual.TextStim(win, text=currPageItems[iItem].replace('_',' '), pos=allWordPosValues[iItem], color=textColor, height=itemHeight, alignText='Left'))
                allBoxes.append(visual.ShapeStim(win, vertices=((-.5, -.3), (-.5, .3), (.5, .3), (.5, -.3)), pos=allSquarePosValues[iItem], size=(squareSize*.5,squareSize*1.5), opacity=100, fillColor=None,lineColor=shapeLineColor,lineWidth=3.0))
        else:
            allScreenWords.append(visual.TextStim(win, text=currPageItems[iItem].replace('_',' '), pos=allWordPosValues[iItem], color=textColor, height=itemHeight, alignText='Left'))
            allBoxes.append(visual.ShapeStim(win, vertices=((-.5, -.3), (-.5, .3), (.5, .3), (.5, -.3)), pos=allSquarePosValues[iItem], size=(squareSize*.5,squareSize*1.5), opacity=100, fillColor=None,lineColor=shapeLineColor,lineWidth=3.0))

    # # Prep Continue Button
    instructTxt1 = visual.TextStim(win, text=instr1, pos=(-.7,0.1), color=textColor, height=0.09, wrapWidth=6) #height=0.12, alignText='Left')
    instructTxt2 = visual.TextStim(win, text=instr2, pos=(-.7,0.05), color=instr2color, height=0.09, wrapWidth=6) #height=0.12, alignText='Left')
    instructTxt3 = visual.TextStim(win, text=instr3, pos=(-.7,-.85), color=textColor, height=0.09, wrapWidth=6) #height=0.12, alignText='Left')   
    continueText = visual.TextStim(win, text='Click here to continue', pos=(0.7, -.85), color=screenColor, height=0.08) 
    exitShape = visual.ShapeStim(win, vertices=((-.5, -.3), (-.5, .3), (.5, .3), (.5, -.3)), pos=(1, 1), size=(.35, .35), opacity=100, fillColor=screenColor, lineColor=None,lineWidth=4.0,name='exitShape')
    continueShape=exitShape
    ...
    while continueChosen == False:
        ...
        for s in range(0,len(allBoxes)):
            if  Mouse.isPressedIn(allBoxes[s]): #button1shp.contains(Mouse): #Mouse.isPressedIn(button1shp):
                mouseDown=Mouse.getPressed()[0]
                if mouseDown and not previousMouseDown: #Only add to list if new click (otherwise, outputs each time frame refreshes, even if in the same button click)
                    if itemsChosen[s]==0: #item hasn't been chosen yet    
                        ...
                        core.wait(.01) #reset button press
                    elif itemsChosen[s]==1: #item was already chosen and is being de-selected
                        itemsChosen[s]=0
...
def presentRefinedItemList(items,pauseTime,instr1,instr2,instr2color):
    ...
    if numItems<=12:
        ...
        xPosCenter = (2/numCols)/2-.5*1.25 #2/ since distance of screen units (+1-->-1), /2 for middle of word, *1.5 for scale
        refItemHeight=itemHeight
        colGap=0.6
    ...
    yPosCenter = ((numRows*refItemHeight)/2)+refItemHeight*5 #items*height gives total screen needed, /2 to split equally b/w top and bottom half of screen
  
    allWordPosValues=[]
    allSquarePosValues=[]
    for iXpos in range(numCols):
        currXpos=xPosCenter+.1+colGap*(iXpos) #first column starts at center, next shifts right
        for iYpos in range(numRows):
            currYpos=yPosCenter-(squareOutlineSize/2)-squareOutlineSize*1.25*(iYpos)
    
            allWordPosValues.append((currXpos+(.35),currYpos)) #[iYpos]))
            allSquarePosValues.append((currXpos-.2,currYpos))
    
    allScreenWords = []
    allBoxes = []
    allChoices = []
    for iItem in range(0,len(items)): #(totalPicCount,totalPicCount+(numPicRows*numPicsPerRow)): #len(allDistTrialStim)):
        if numCols > 1 and len(items[iItem]) > 16: # for long labels, decrease font size
            currItemHeight=refItemHeight-0.01 
        else:
            currItemHeight=refItemHeight
        allScreenWords.append(visual.TextStim(win, text=items[iItem].replace('_',' '), pos=allWordPosValues[iItem], color=instr2color, height=currItemHeight, alignText='Left'))
        allBoxes.append(visual.ShapeStim(win, vertices=((-.5, -.3), (-.5, .3), (.5, .3), (.5, -.3)), pos=allSquarePosValues[iItem], size=(squareSize*.5,squareSize*1.5), opacity=100, fillColor=None,lineColor=shapeLineColor,lineWidth=3.0))
        allChoices.append(visual.TextStim(win,text=' ',color=textColor,height=refItemHeight,bold=True))
    ...
    while continueChosen == False:
        ...
        if sum(itemsChosen) != 0: #if they've clicked something, give option to reset
            resetText = visual.TextStim(win, text='RESET', pos=(-0.7, -.85), color='white', height=0.08) 
            resetShape = visual.ShapeStim(win, vertices=((-.5, -.3), (-.5, .3), (.5, .3), (.5, -.3)), pos=(-0.7, -.85), size=(.3, .2), opacity=100, fillColor='black', lineColor=shapeLineColor,lineWidth=4.0,name='continueShape')
...
# --- Setup the Window ---
win = visual.Window(
    size=screenSize, fullscr=fullScreenChoice, screen=screenChoice, 
    color='lightgray', colorSpace='rgb', units='norm')
...
instructions1 = ('In this experiment, you will listen to sounds.' +
...
for iPage in range(numPages):
    ...
    mostTriggeringList_page=presentItemList(iPage,pauseTime,instr1,instr2,'red',instr3)
    for iItem in range(len(mostTriggeringList_page)):
        if mostTriggeringList_page[iItem]==1:
             mostTriggeringList.append(uniqueSoundLabels[iItem+iPage*numItemsPerPage])
...
[mostTriggeringList_refined,mostTriggering_ranks]=presentRefinedItemList(mostTriggeringList,pauseTime,instr1,instr2,'red')
for iItem in range(len(mostTriggeringList)):
    if mostTriggeringList_refined[iItem]==1:
        refinedMostTriggeringList.append([mostTriggering_ranks[iItem],mostTriggeringList[iItem]])
...
for iPage in range(numPages):
    instr3='Page '+str(iPage+1)+'/'+str(numPages)
    leastTriggeringList_page=presentItemList(iPage,pauseTime,instr1,instr2,'green',instr3)
    for iItem in range(len(leastTriggeringList_page)):
        if leastTriggeringList_page[iItem]==1:
             leastTriggeringList.append(uniqueSoundLabels[iItem+iPage*numItemsPerPage])

instr1='Please rank the \n\nsounds to you. \n\n1 = more neutral\n5 = less neutral \n\n\nOnce you have \nselected your top 5, \ncontinue to the \nnext page.'
instr2='5 MOST NEUTRAL\n\n\n\n\n\n\n\n\n\n'

refinedLeastTriggeringList=[]
[leastTriggeringList_refined,leastTriggering_ranks]=presentRefinedItemList(leastTriggeringList,pauseTime,instr1,instr2,'green')
for iItem in range(len(leastTriggeringList)):
    if leastTriggeringList_refined[iItem]==1:
        refinedLeastTriggeringList.append([leastTriggering_ranks[iItem],leastTriggeringList[iItem]])
...
instructTxt1 = visual.TextStim(win, text=compileInstructions, pos=(0,0), color=textColor, height=0.09, wrapWidth=6) #height=0.12, alignText='Left')
exitShape = visual.ShapeStim(win, vertices=((-.5, -.3), (-.5, .3), (.5, .3), (.5, -.3)), pos=(1, 1), size=(.35, .35), opacity=100, fillColor=screenColor, lineColor=None,lineWidth=4.0,name='exitShape')
...
while soundsCompiled == False:
    ...
    filename = dataDir +u'\%s_%s_%s_%s' % (expInfo['participant'], expName,'SoundSelect',expInfo['date'] + '.txt')
    ...
    for iMost in mostTriggeringList:
        with open(filename,"a") as textfile:
            print('Trigger\t\t',str(round(mostTriggering_ranks[rankPos]))+'\t',iMost, file=textfile)
    ...
    for iLeast in leastTriggeringList:
        with open(filename,"a") as textfile:
            print('Neutral\t\t',str(round(leastTriggering_ranks[rankPos]))+'\t',iLeast, file=textfile)
